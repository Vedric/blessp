...
async function auditRoute(route, mobile, key) {
  // Give each measurement its own browser process and profile: trace recording,
  // renderer state and caches must not leak from the previous route.
  const chromeProfile = fs.mkdtempSync(path.join(os.tmpdir(), 'blessp-lighthouse-'));
  let chrome;
  try {
    // Launch through Playwright; chrome-launcher otherwise converts Linux
    // profile paths to Windows paths under WSL.
    ...
    if (!result) throw new Error(`Lighthouse returned no result for ${key}`);
    // Keep failed recordings too. Throwing before saving erased the evidence
    // needed to diagnose the CI's NO_NAVSTART instrumentation failure.
    fs.writeFileSync(path.join(output, `${key}.html`), result.report[0]);
    fs.writeFileSync(path.join(output, `${key}.json`), result.report[1]);
    if (result.artifacts.Trace) await saveTrace(result.artifacts.Trace, path.join(output, `${key}.trace.json`));
    if (result.artifacts.DevtoolsLog) await saveDevtoolsLog(result.artifacts.DevtoolsLog, path.join(output, `${key}.devtoolslog.json`));
    if (result.lhr.runtimeError) throw new Error(`Lighthouse failed for ${key}: ${result.lhr.runtimeError.message}`);
    return result;
  } finally {
    try { await chrome?.close(); } finally { fs.rmSync(chromeProfile, { recursive: true, force: true }); }
  }
}
try {
  let ready = false;
  ...
  for (const [route, mobile] of [['/', false], ['/shop', false], ['/signin', false], ['/', true]]) {
    ...
    const result = await auditRoute(route, mobile, key);
