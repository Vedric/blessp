...
describe('Auth API integration tests', () => {
  describe('POST /api/v1/auth/register', () => {
    it('creates an unverified user and returns the generic 202 without credentials', async () => {
      ...
      expect(res.body.data.message).toMatch(/check your inbox/i);
      expect(res.body.data.tokens).toBeUndefined();
      expect(res.headers['set-cookie']).toBeUndefined();
      const stored = await prisma.user.findUniqueOrThrow({ where: { email: 'newuser@example.com' } });
      expect(stored.emailVerifiedAt).toBeNull();

    });

    it('returns a generic 202 when email is already registered (no enumeration)', async () => {
      if (!dbAvailable) return;
