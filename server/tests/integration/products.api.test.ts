...
describe('Products API integration tests', () => {
  ...
  describe('GET /api/v1/products/filters', () => {
    it('aggregates distinct facets and prices without drafts, deleted products or cross-products', async () => {
      await prisma.product.createMany({ data: [
        { name: 'A', price: 0, category: 'hoodies', colors: ['black', 'blue'], sizes: ['S', 'M'] },
        { name: 'B', price: 12000, category: 'hoodies', colors: ['blue'], sizes: ['M', 'L'] },
        { name: 'C', price: 5000, category: null, colors: [], sizes: [] },
        { name: 'Draft', price: 99999, category: 'private', colors: ['secret'], sizes: ['XXL'], isActive: false },
        { name: 'Deleted', price: 99999, category: 'deleted', colors: ['deleted'], deletedAt: new Date() },
      ] });
      const res = await request(app).get('/api/v1/products/filters').expect(200);
      expect(res.body.data).toEqual({ categories: ['hoodies'], colors: ['black', 'blue'], sizes: ['L', 'M', 'S'], priceRange: { min: 0, max: 12000 } });
    });

    it('returns empty facets and a zero price range for an empty catalogue', async () => {
      const res = await request(app).get('/api/v1/products/filters').expect(200);
      expect(res.body.data).toEqual({ categories: [], colors: [], sizes: [], priceRange: { min: 0, max: 0 } });
    });
  ...
  it('keeps page boundaries stable when all prices and timestamps tie', async () => {
    const createdAt = new Date('2026-01-01T00:00:00Z');
    const ids = Array.from({ length: 25 }, (_, i) => `00000000-0000-4000-8000-${String(i).padStart(12, '0')}`);
    await prisma.product.createMany({ data: [...ids].reverse().map(id => ({ id, name: 'Tie', price: 100, createdAt })) });
    for (const sort of ['price:asc', 'createdAt:desc']) {
      const pages = await Promise.all([1, 2, 3].map(page => request(app).get(`/api/v1/products?perPage=10&page=${page}&sort=${sort}`).expect(200)));
      expect(pages.flatMap(res => res.body.data.map((product: { id: string }) => product.id))).toEqual(ids);
    }
  });
