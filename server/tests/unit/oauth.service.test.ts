  ...
  describe('verifyGoogleToken', () => {
    it('returns user info on a valid Google access token', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://www.googleapis.com/oauth2/v3/tokeninfo?access_token=valid-access-token',
    ...
    it('throws UnauthorizedError when the Google token audience does not match our client id', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          aud: 'some-other-google-client-id',
          sub: 'google-uid-999',
          exp: '9999999999',
      ...
      const error = await service.verifyGoogleToken('stolen-token').catch((e) => e);

      expect(error).toBeInstanceOf(UnauthorizedError);
      expect(error.message).toMatch(/different client/i);
    });
