...
export class LoyaltyService {
  ...
  async getBalance(userId: string): Promise<LoyaltyBalanceResponse> {
    ...
    const redeemableUnits = Math.max(0, Math.floor(points / POINTS_PER_REDEMPTION_UNIT));
  ...
  async redeemPoints(userId: string, dto: RedeemPointsDto): Promise<LoyaltyTransactionResponse> {
    if (!Number.isSafeInteger(dto.points) || dto.points <= 0 || dto.points % POINTS_PER_REDEMPTION_UNIT !== 0) {
    ...
    const transaction = await this.loyaltyRepository.redeem({
      discountCents,
      userId,
      points: -dto.points,
      type: 'redeemed',
