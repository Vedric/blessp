...
export const UpdateVariantsSchema = z.object({
  variants: z.array(
    z.object({
      ...
      color: z.string().max(50).trim(),
      stock: z.number().int('Stock must be an integer.').nonnegative('Stock cannot be negative.').max(1000000),
      expectedStock: z.number().int().min(0).max(2147483647).optional(),
