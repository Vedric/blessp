...
export class ProductsService {
  ...
  async getProduct(id: string, includeInactive = false): Promise<ProductResponse> {
  ...
  async createProduct(dto: CreateProductDto): Promise<ProductResponse> {
    ...
    await this.cache.deleteByPattern('products:*');
    return this.toProductResponse(product);
  }
  ...
  async getVariants(productId: string, includeInactive = false): Promise<ProductVariantResponse[]> {
    const product = await this.productsRepository.findById(productId);
    if (!product || (!includeInactive && !product.isActive)) {
      throw new NotFoundError('Product', productId);
    }
  ...
  async updateVariants(productId: string, updates: UpdateVariantInput[], actorId?: string): Promise<ProductVariantResponse[]> {
    const product = await this.productsRepository.findById(productId);
    if (!product) {
      throw new NotFoundError('Product', productId);
    }

    if (actorId) await this.variantsRepository.upsertMany(productId, updates, actorId);
    else await this.variantsRepository.upsertMany(productId, updates);
    await this.cache.deleteByPattern('products:*');
    const variants = await this.variantsRepository.findByProductId(productId);
    return variants.map(this.toVariantResponse);
  }
