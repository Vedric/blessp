...
export class CouponsController {
  ...
  validate = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const result = await this.couponsService.validateCoupon(dto.code, dto.orderTotalCents, (req as AuthenticatedRequest).user?.userId);
  ...
  apply = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const result = await this.couponsService.applyCoupon(dto.code, dto.orderTotalCents, (req as AuthenticatedRequest).user?.userId);
