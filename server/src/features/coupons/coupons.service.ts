...
export class CouponsService {
  ...
  async validateCoupon(code: string, orderTotalCents: number, userId?: string): Promise<ApplyCouponResult> {
    ...
    if (coupon.userId && coupon.userId !== userId) throw new ValidationError('This coupon is unavailable for this account.');
  ...
  async applyCoupon(code: string, orderTotalCents: number, userId?: string): Promise<ApplyCouponResult> {
    // Preview only. Consumption belongs to the order transaction.
    return this.validateCoupon(code, orderTotalCents, userId);
  }

  async createCoupon(dto: CreateCouponDto): Promise<CouponResponse> {
    if (dto.discountType === 'percentage' && dto.discountValue > 100) throw new ValidationError('Percentage must not exceed 100.');
  ...
  private calculateDiscount(
    ...
    if (discountType === 'percentage') {
      return Math.min(orderTotalCents, Math.round((orderTotalCents * discountValue) / 100));
    }
