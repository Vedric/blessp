...
test.describe('Authentication', () => {
  test.describe('Sign Up', () => {
    test('displays validation errors for empty fields on submit', async ({ page }) => {
      ...
      const errorMessages = page.locator('[aria-invalid=true], .text-red-500');
    ...
    test('validates password strength requirements', async ({ page }) => {
      ...
      await authPage.signUpPassword.fill(`TestA1-${crypto.randomUUID()}`);
    ...
    test('shows password match confirmation', async ({ page }) => {
      ...
      const password = `TestA1-${crypto.randomUUID()}`;
      ...
      const matchText = page.getByText('Passwords match', { exact: true });
    ...
    test('shows mismatch error when passwords differ', async ({ page }) => {
      ...
      await authPage.signUpPassword.fill(`TestA1-${crypto.randomUUID()}`);
      await authPage.signUpConfirmPassword.fill(`DifferentB2-${crypto.randomUUID()}`);
      await authPage.signUpConfirmPassword.blur();

      // The confirm-password field surfaces a mismatch message
  ...
  test.describe('Protected Routes', () => {
    ...
    test('does not redirect checkout to sign in (guest checkout is public)', async ({ page }) => {
      // Checkout is a public route: an unauthenticated visitor is never sent to
      // sign-in. With an empty cart the page bounces to the shop, so we assert
      // the absence of a sign-in redirect rather than a fixed destination.
