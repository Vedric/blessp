...
const sectionKeys = ['identity', 'dataCollected', 'purposes', 'legalBases', 'processors', 'transfers', 'retention', 'rights', 'cookies', 'children', 'changes'] as const;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-4xl px-4 pt-6 sm:px-6 lg:px-8">
        <Breadcrumbs items={[{ label: t('common.home'), href: '/' }, { label: t('privacy.title') }]} />
      </div>
          ...
          {sectionKeys.map((key) => (
            ...
            >
              ...
              <motion.div
                ...
                variants={fadeUp}
              >
                {(t(`privacy.sections.${key}.content`) as string).split('\n\n').map((paragraph, pIndex) => (
                ...
                {key === 'identity' && (
                  <div className="mt-4 rounded-lg border border-neutral-200 bg-neutral-50 p-6">
                    ...
                    <p className="mt-1 text-neutral-600">{t('privacy.officer.role')}</p>
                    <p className="mt-2 text-neutral-600">
                      {t('common.email')}:{' '}
                      <a
                        href="mailto:privacy@blessp.com"
                        className="text-brand-700 underline underline-offset-4 transition-colors hover:text-brand-700"
                      >
                        privacy@blessp.com
                      </a>
                    </p>
                    <p className="mt-1 text-neutral-600">{t('privacy.officer.address')}</p>
                  </div>
                )}
              </motion.div>
            </motion.div>
