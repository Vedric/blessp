...
function ToggleSwitch({ label, enabled, onChange, disabled }: ToggleSwitchProps) {
  return (
    <button
      ...
      className={cn(
        'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#a07a52] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50',
        enabled ? 'bg-[#a07a52]' : 'bg-neutral-200',
      )}
    >
      <span
        className={cn(
