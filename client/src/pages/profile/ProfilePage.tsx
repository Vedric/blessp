...
export default function ProfilePage() {
  ...
  const handlePasswordChange = async (e: FormEvent) => {
    ...
    try {
      await api.post('/users/change-password', { currentPassword, newPassword, mfaToken: securityCode || undefined });
      setPasswordSuccess(t('profile.passwordUpdated'));
      setCurrentPassword('');
      setNewPassword('');
      setIsChangingPassword(false);
      await logout(); navigate('/signin');
    } catch (err: unknown) {
      const apiErr = err as { message?: string };
  ...
  useDialogFocus(showDeleteModal, deleteRef, () => setShowDeleteModal(false));
  ...
  const handleDeleteAccount = async (e: FormEvent) => {
    e.preventDefault();
    setDeleteError('');
    if (!deletePassword && !securityCode) {
      setDeleteError(t('profile.deleteAccount.passwordRequired'));
      return;
    }
    setIsDeleting(true);
    try {
      await api.delete('/users/account', { password: deletePassword || undefined, mfaToken: securityCode || undefined });
      await logout();
      navigate('/');
    } catch (err: unknown) {
      const apiErr = err as { message?: string };
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              {!isEditingProfile && (
                <button
                  ...
                  className="text-xs font-medium tracking-widest text-brand-700 uppercase"
                >
                  {t('common.edit')}
                </button>
              )}
            ...
            {isEditingProfile && (
              <form onSubmit={handleProfileSave} className="mt-6 space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label htmlFor="profile-first-name" className="block text-xs font-medium tracking-widest text-neutral-500 uppercase">
                  ...
                  <div>
                    <label htmlFor="profile-last-name" className="block text-xs font-medium tracking-widest text-neutral-500 uppercase">
                      {t('checkout.lastName')}
                    </label>
          ...
          <div className="mt-6 border border-neutral-100 p-6">
            ...
            {isChangingPassword && (
              <form onSubmit={handlePasswordChange} className="mt-6 space-y-4">
                <label className="block text-sm">{t('auth.signIn.mfaLabel', { defaultValue: 'Authenticator or backup code (if enabled)' })}<input className="mt-2 w-full border p-3" value={securityCode} onChange={(e) => setSecurityCode(e.target.value)} autoComplete="one-time-code" /></label>
                {passwordError && (
                ...
                <div>
                  <label htmlFor="profile-current-password" className="block text-xs font-medium tracking-widest text-neutral-500 uppercase">
                ...
                <div>
                  <label htmlFor="profile-new-password" className="block text-xs font-medium tracking-widest text-neutral-500 uppercase">
              ...
              <Link
                ...
                className={cn(
                  ...
                  item.to === '/profile/loyalty'
                    ? 'border-[#a07a52]/20 bg-gradient-to-r from-[#a07a52]/5 to-transparent'
                    : 'border-neutral-100',
                )}
              >
                <div className="flex items-center gap-3">
                  <item.icon
                    className={cn(
                      'h-5 w-5',
                      item.to === '/profile/loyalty' ? 'text-[#80603c]' : 'text-neutral-500',
                    )}
                  />
                  <span className="text-sm font-medium text-neutral-900">
                    {item.label}
                  ...
                  {item.to === '/profile/loyalty' && loyaltyBalance && (
                    <span className="text-2xs font-medium text-[#80603c]">
            ...
            {showDeleteModal && (
              ...
              >
                <motion.div
                  ...
                  ref={deleteRef}
                  role="dialog" aria-modal="true" aria-labelledby="delete-dialog-title" tabIndex={-1}
                  className="max-h-[90vh] w-full max-w-md overflow-y-auto bg-white p-8"
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <h3 id="delete-dialog-title" className="font-display text-lg font-medium text-neutral-900">
                ...
                {deleteError && (
                    ...
                    <div>
                      <label htmlFor="delete-password" className="block text-xs font-medium tracking-widest text-neutral-500 uppercase">
                        {t('profile.deleteAccount.passwordOrSocialFactor')}
                      </label>
                      <div className="relative">
                        ...
                        <button
                          ...
                          aria-label={showDeletePassword ? t('auth.hidePassword', { defaultValue: 'Hide password' }) : t('auth.showPassword', { defaultValue: 'Show password' })}
                          className="absolute right-1 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center text-neutral-500 hover:text-neutral-600"
                        >
                    ...
                    <div className="flex gap-3">
                      <button
                        type="submit"
                        disabled={isDeleting || (!deletePassword && !securityCode)}
