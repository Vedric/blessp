...
const statusConfig: Record<string, { label: string; className: string; dot: string }> = {
  paid: { label: 'Paid', className: 'bg-green-50 text-green-700 border-green-200', dot: 'bg-green-500' },
    ...
    const fetchOrders = async () => {
      ...
      try {
        const res = await api.getRaw<PaginatedResponse<Order>>(
          `/orders/mine?page=${page}&perPage=10`,
        );
        setOrders(res.data);
            ...
            >
              <div className="relative">
                ...
                <motion.div
                  ...
                  className="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-[#a07a52]/10"
                >
                  <Sparkles className="h-3.5 w-3.5 text-[#80603c]" />
                </motion.div>
              </div>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link
                  to="/shop"
                  className="inline-flex items-center justify-center gap-2 bg-neutral-900 px-8 py-3.5 text-sm font-medium tracking-widest text-white uppercase transition-all hover:bg-[#a07a52] hover:text-neutral-950"
                >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            ...
                            {t(`orders.status.${order.status}`, { defaultValue: status.label })}
                          </span>
                        </div>
