...
export default function AdminReviewsPage() {
  ...
  useDialogFocus(Boolean(deleteTarget), dialogRef, () => { if (!isDeleting) setDeleteTarget(null); });
    ...
    <div className="min-h-screen px-4 pt-32 pb-24 sm:px-6">
            ...
            >
              ...
              <div className="hidden md:block">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-neutral-100 text-left text-2xs font-medium tracking-widest text-neutral-500 uppercase">
                      ...
                      >
                        ...
                        <td className="py-4">
                          <button
                            ...
                            aria-label={`${t('admin.reviews.deleteReview')}: ${review.title || review.productName}`}
                            className="p-1.5 text-neutral-500 transition-colors hover:text-red-600"
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <button
                        onClick={() => setDeleteTarget(review)}
                            aria-label={`${t('admin.reviews.deleteReview')}: ${review.title || review.productName}`}
                        className="p-1 text-neutral-500 transition-colors hover:text-red-600"
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
      ...
      <AnimatePresence>
        {deleteTarget && (
          ...
          >
            <motion.div
              ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="delete-review-title" tabIndex={-1}
              className="max-h-[90vh] overflow-y-auto w-full max-w-md bg-white p-8"
            ...
            >
              <div className="flex items-start justify-between">
                <h3 id="delete-review-title" className="text-lg font-medium text-neutral-900">
                  {t('admin.reviews.deleteReview')}
                </h3>
                <button
                  onClick={() => !isDeleting && setDeleteTarget(null)}
                  aria-label={t('common.close', { defaultValue: 'Close' })}
                  className="flex h-11 w-11 items-center justify-center text-neutral-500 hover:text-neutral-600"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="mt-2">
                ...
                {deleteTarget.comment && (
                  <p className="mt-1 line-clamp-2 text-sm text-neutral-500 italic">
