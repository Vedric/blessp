...
export function Header() {
  ...
  useDialogFocus(mobileMenuOpen, mobileRef, () => setMobileMenuOpen(false));
  ...
  return (
    <>
      <header onFocusCapture={() => setVisible(true)}
        className={cn(
          'fixed top-0 left-0 right-0 z-40 bg-[#fcfbf8]/95 backdrop-blur-md border-b border-neutral-200 transition-transform duration-300 ease-out-expo',
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            ...
            />
            <motion.nav
              ref={mobileRef} role="dialog" aria-modal="true" aria-label={t('nav.menu', { defaultValue: 'Menu' })} tabIndex={-1}
              initial={{ x: '-100%' }}
              animate={{ x: 0 }}
              exit={{ x: '-100%' }}
              transition={{ duration: 0.35, ease: [0.19, 1, 0.22, 1] }}
              className="fixed left-0 top-0 z-50 flex h-full w-72 max-w-full flex-col overflow-y-auto overscroll-contain bg-white px-8 py-6"
            >
              ...
              <button
                ...
                className="mb-4 flex w-full shrink-0 items-center gap-3 rounded border border-neutral-200 px-4 py-3 text-sm text-neutral-500 transition-colors hover:border-neutral-300 hover:text-neutral-700"
              >
                <Search size={16} />
                ...
                {isAuthenticated ? (
                  <>
                    ...
                    <motion.button
                      ...
                      className="text-left text-sm font-medium uppercase tracking-widest text-neutral-500 transition-colors hover:text-neutral-900"
                    >
                      {t('common.signOut')}
                    </motion.button>
                  </>
              ...
              <div className="mt-auto flex shrink-0 items-center gap-4 border-t border-neutral-100 pt-6">
                <LanguageSwitcher placement="above" />
                <span className="text-neutral-200">|</span>
                <CurrencySelector placement="above" />
              </div>
            </motion.nav>
          </>
        )}
