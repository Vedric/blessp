    ...
    <article className="product-card group relative min-w-0">
      <Link to={`/products/${product.id}`} className="block" aria-label={product.name}>
        <div className="relative aspect-[3/4] overflow-hidden bg-[#efeee9]">
          <ProductImage src={picture} alt={product.name} className="h-full w-full object-cover transition-transform duration-500 ease-out group-hover:scale-[1.04] motion-reduce:transform-none" />
        ...
        <div className="flex items-start justify-between gap-2 border-b border-neutral-200 pb-4 pt-4">
          <div className="min-w-0"><Heading className="text-sm font-medium leading-relaxed text-neutral-900 [overflow-wrap:anywhere]">{product.name}</Heading><p className="mt-1 text-xs text-neutral-600">{formatPrice(product.price)}</p></div>
