...
export function CompleteLook({ productId, currentProduct }: CompleteLookProps) {
  ...
  const handleShopTheSet = async (suggestion: Product) => {
    ...
    try {
      ...
      await addToCart(currentProduct, currentSize, currentColor, 1);
      await addToCart(suggestion, suggestedSize, suggestedColor, 1);
    } catch {
      // Cart context handles error display
    } finally {
      setAddingSet(false);
  ...
  return (
    ...
    >
      ...
      {/* Set pricing banner */}
      <motion.div
        className="mt-8 flex flex-wrap items-center gap-4 border border-[#a07a52]/30 bg-[#a07a52]/5 px-6 py-4"
        variants={fadeSlideUp}
      >
        <Sparkles className="h-5 w-5 text-[#80603c]" />
        <div className="flex flex-wrap items-baseline gap-2">
          ...
          <span className="text-lg font-medium text-[#80603c]">
            {formatPrice(setTotalAfterDiscount)}
          </span>
          <span className="rounded-sm bg-[#a07a52]/10 px-2 py-0.5 text-xs font-medium text-[#80603c]">
        ...
        <motion.button
          ...
          disabled={addingSet}
          className="ml-auto flex items-center gap-2 bg-neutral-900 px-5 py-2.5 text-xs font-medium tracking-widest text-white uppercase transition-all hover:bg-[#a07a52] hover:text-neutral-950 disabled:opacity-50"
