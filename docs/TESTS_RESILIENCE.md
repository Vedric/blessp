...
| Photo absente ou introuvable, galerie et cartes produit à 320 px, récupération après correction de l’URL | `product-images.spec.ts`, `experience.spec.ts` | Validation des photos et URLs du catalogue commercial |
...
- Les produits sans photo pouvaient afficher une image cassée, notamment dans les recommandations. Un composant partagé présente maintenant un remplacement accessible pour les images absentes ou en erreur, dans la galerie, les listes, la recherche, le comparateur, le panier, le checkout et les aperçus administrateur. Les petits aperçus conservent une description accessible sans ajouter de texte illisible dans leur espace réduit. Les tests couvrent les deux langues, le mobile et le retour à une image disponible.
- Le bandeau de comparaison pouvait recouvrir le bouton de paiement du panier sur mobile. Son niveau d’affichage reste maintenant sous les fenêtres modales ; un test sélectionne deux articles à comparer, les ajoute au panier, puis rejoint le checkout par un clic utilisateur normal.
- Les boutons de sauvegarde et d’annulation du formulaire produit débordaient à 320 px en français. Ils sont disposés verticalement sur petit écran, avec contrôle du débordement dans le scénario administratif.
- L’éditeur ne montrait pas une photo principale absente du tableau de galerie et conservait l’ancienne photo principale après suppression. Toutes les photos sont maintenant visibles, sans doublon ; supprimer la principale choisit la première photo restante, et supprimer la dernière envoie explicitement `null` à la modification. La première photo ajoutée à un nouveau produit devient sa principale. Un parcours FR/EN contrôle les valeurs réellement enregistrées, la suppression complète, le remplacement et la création.
- Une catégorie longue mais valide débordait de la liste administrative et chevauchait les actions. La ligne réserve maintenant une rangée aux actions sur mobile et permet aux métadonnées de revenir à la ligne. Un test dédié utilise les longueurs et le prix maximum autorisés à 320, 390, 768 et 1440 px, en FR/EN, puis ouvre réellement l’édition.
...
L’exploration complémentaire du 26 septembre fige temporairement Playwright sur `1.64.0-alpha-2026-09-26` pour embarquer Firefox r1551 et le correctif de collision des canaux du pilote ([défaut amont](https://github.com/microsoft/playwright/issues/42731)). La version stable 1.63.0 ne l’inclut pas. Remplacer cette préversion datée par une stable contenant le correctif après validation de la matrice ; conserver COOP et les contrôles d’intermittence. Le [rapport complémentaire](audits/2026-09-26/EXPLORATION_INTERFACE.md) détaille les traces, les essais locaux et les limites de l’attribution causale.
